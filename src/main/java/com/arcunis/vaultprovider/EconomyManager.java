...
public class EconomyManager {
    ...
    public static double depositAcc(UUID uuid, double amount) {
        ...
        try (PreparedStatement statement = db.conn.prepareStatement(updateQuery)) {
    ...
    public static double withdrawAcc(UUID uuid, double amount) {
        ...
        try (PreparedStatement selectStatement = db.conn.prepareStatement(selectQuery)) {
        ...
        try (PreparedStatement updateStatement = db.conn.prepareStatement(updateQuery)) {
    ...
    public static void deleteBank(String name) {
        ...
        try (PreparedStatement statement = db.conn.prepareStatement(deleteQuery)) {
            statement.setString(1, name);
            statement.executeUpdate();
            db.close();
        } catch (SQLException e) {
    ...
    public static UUID getBankOwner(String name) {
        ...
        try (PreparedStatement statement = db.conn.prepareStatement(query)) {
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                if (resultSet.next()) {
                    db.close();
                    return UUID.fromString(resultSet.getString("owner"));
                } else {
                    db.close();
                    throw new RuntimeException("Bank not found with name: " + name);
                }
        ...
        try (PreparedStatement statement = db.conn.prepareStatement(query)) {
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                while (resultSet.next()) {
                    members.add(UUID.fromString(resultSet.getString("member_uuid")));
                }
            }
        } catch (SQLException e) {
            db.close();
